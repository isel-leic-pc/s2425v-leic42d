...
class HttpRequestsStateMachine(
    ...
    private lateinit var group: AsynchronousChannelGroup
    ...
    private fun state4(): NextState {
        responseByteArray = ByteArray(1024)
    ...
    private fun <T> startAsynchronous(
        ...
        val completionHandler = object : CompletionHandler<T, Void?> {
            override fun completed(result: T, attachment: Void?) {
                logger.info("completion with $result bytes on thread ${Thread.currentThread().name}")
                val holder = tls.get()
                if (holder != null) {
                    holder.value = result
                } else {
        ...
        logger.info("start async operation at ${Thread.currentThread().name}")
        val holder = Holder()
        tls.set(holder)
        try {
...
private fun getHttpBin(path: String, continuation: CompletionHandler<String, Void?>) {
...
private fun main() {
    ...
    val completionHandler = object : CompletionHandler<String, Void?> {
